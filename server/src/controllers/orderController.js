...
export const orderController = {
  ...
  async warehouseProducts(req, res) {
    try {
      const branchId = Number(req.query.branch_id || req.query.warehouse_id);
      if (!branchId) return res.status(400).json({ message: "branch_id (or warehouse_id) is required" });
      res.json({
        data: await orderService.warehouseProducts(req.tenantId, branchId),
      });
    } catch (e) {
      res.status(500).json({ message: e.message });
    }
